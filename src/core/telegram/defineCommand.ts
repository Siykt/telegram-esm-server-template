...
export interface TGCommandContext<T extends TGCommandArgs> {
  command: string
...
interface TGCommand<T extends TGCommandArgs> {
  command: string
  description: string
  templatePath?: string
...
function getArgsValue(type: TGCommandArgTypes, value: string) {
  return type === 'boolean' ? value === 'true' : type === 'number' ? Number.parseInt(value) : value
}
  ...
  for (const [key, { type = 'string', required, description, ...fileInfo }] of Object.entries(args)) {
    await new Promise<void>((resolve, reject) => {
      const callback = async (newMsg: TelegramBot.Message) => {
        ...
        // skip
        if (newMsg.text === '/skip' && !required)
          return done()

        // check if the message is a command
        if (/^\/(?:yes|no)/.test(newMsg.text ?? '')) {
          if (type !== 'boolean')
            return done(new Error('Boolean type required'))
        ...
        // stop on new command
        if (newMsg.text?.startsWith('/'))
          return done(new Error('New command detected'))

        if (type === 'file') {
          if (!newMsg.document)
            return done(new Error('File is required'))
        ...
        if (!newMsg.text)
          return done(new Error('Text is required'))
      ...
      if (type === 'file') {
        if (!fileInfo.templateFilepath)
          return reject(new Error('Filepath is required for file type'))
  ...
  try {
    ...
    if (middlewares) {
      for (const middleware of middlewares) {
        ...
        if (aborted) {
          if (abortMsg)
            await botClient.sendMDMessage(msg.chat.id, abortMsg)
