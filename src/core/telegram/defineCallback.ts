...
interface TGQueryCallbackContext<T extends TGQueryArgs> {
  query: string
...
interface TGQueryCallbackConfig<T extends TGQueryArgs> {
  query: string
  text: string
...
export function setupTGQueries() {
  botClient.bot.on('callback_query', async (query) => {
    ...
    if (!command)
      return throwError('No query')

    const callback = queryCallbacks.get(command)
    if (!callback)
      return throwError('Query not found')
    ...
    if (argsKey) {
      ...
      if (cacheParams)
        Object.assign(parserArgs, JSON.parse(cacheParams))
    }
...
>(config: TGQueryCallbackConfig<T>): R {
  ...
  if (config.args) {
    if (config.query.length > 64 - 9)
      throw new Error('Query length must be less than 55 characters(9 for key)')
